...
class LibriUMixDataset(Dataset):
    def __init__(
        self,
        json_dir: str = "",
        n_src: int = 3,
        ...
        self.n_src = n_src
        self.test = self.seg_len is None
        mix_json = os.path.join(json_dir, "remix.json")
        sources_json = [
            os.path.join(json_dir, source + ".json") for source in ["s1", "s2", "s3"][:n_src]
        ]
        ...
        for src_json in sources_json:
            if os.path.exists(src_json):
                ...
                sources_infos.append([])

        self.mix = []
        self.sources = []
        

        if self.n_src in [2, 3]:  # 支持2或3个源
            orig_len = len(mix_infos)
            drop_utt, drop_len = 0, 0
            if not self.test:
                for i in range(len(mix_infos) - 1, -1, -1):  # Go backward
...
class LibriUMixModuleRemix(LightningDataModule):
    def __init__(
        self,
        train_dir: str,
        valid_dir: str,
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        if not self.data_train and not self.data_val and not self.data_test:
            self.data_train = LibriUMixDataset(
            ...
            self.data_val = LibriUMixDataset(
            ...
            self.data_test = LibriUMixDataset(
